...
layui.define(["laytpl", "laypage", "layer", "form"], function (e) {
    ...
    j.prototype.config = {limit: 30, loading: !0}, j.prototype.render = function () {
        ...
        if (isPhone()) {
            var tableWidth = parseInt(bodyWidth)-30;
        } else {
            var tableWidth = parseInt(bodyWidth) - 246;
        }
        var length = e.cols[0].length;
        var total_width = 0;
        var cel_width;
        for (var i = 0; i < length; i++) {
            console.log(e.cols[0][i]);
            cel_width = parseInt(e.cols[0][i].width);
            if (cel_width < 10) {
                cel_width = 100;
                e.cols[0][i].width = 100;
            }
            total_width += cel_width;
        }
        var beishu = tableWidth / total_width;
        if (beishu > 1) {
            var new_total_width = 0;
            for (var jj = 0; jj < length - 1; jj++) {
                cel_width = e.cols[0][jj].width;
                e.cols[0][jj].width = parseInt(cel_width * beishu);
                new_total_width += e.cols[0][jj].width;
            }
            var last_index = length - 1;
            e.cols[0][last_index].width = tableWidth - new_total_width - length - 1;
        }
        e.width = tableWidth;
        ////
        var t = new j(e);
